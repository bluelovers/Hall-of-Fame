...
<?php
	...
	function SettingShow() {
		...
		if($this->main->record_btl_log) $record_btl_log	= " checked";
		if($this->main->no_JS_itemlist) $no_JS_itemlist	= " checked";
		?>
<h4>Setting</h4>
...
Color : <?php
		...
		foreach($color as $value) {
			$value	= trim($value);
			print("<option value=\"{$value}\" style=\"color:{$value}\" ".($this->main->UserColor == $value?" selected":"").">");
	...
	function SettingProcess() {
		...
		if($_POST["setting01"]) {
			if($_POST["record_battle_log"])
				$this->main->record_btl_log	= 1;
			else
				$this->main->record_btl_log	= false;

			if($_POST["no_JS_itemlist"])
				$this->main->no_JS_itemlist	= 1;
			else
				$this->main->no_JS_itemlist	= false;
		}
		if($_POST["color"]) {
			if(	strlen($_POST["color"]) != 6 &&
				!ereg("^[0369cf]{6}",$_POST["color"]))
				return "error 12072349";
			$this->main->UserColor	= $_POST["color"];
