...
if ($_POST["add"] && isset($_POST["number"]))
{
	...
	foreach ($var as $head)
	{
		for ($i = ROWS; -1 < $i; $i--)
		{
			if ($number == $i) $_POST[$head . $i] = NULL;
			else
				if ($number < $i) $_POST[$head . $i] = $_POST[$head . ($i - 1)];
				else  break;
		}
	}
}
...
for ($i = 0; $i < ROWS; $i++)
{
	...
	foreach ($judgeList as $key => $exp)
	{
		...
		if ($_POST["judge" . $i] == $key) print ('<option value="' . $key . '"' . $css . 'selected>' . $exp["exp"] . '</option>' . "\n");
		else  print ('<option value="' . $key . '"' . $css . '>' . $exp["exp"] . '</option>' . "\n");
	}
	print ("</select>\n");
	print ("</td><td>\n");
	// 数値
	...
	{
		if ($_POST["skill" . $i] == $key) print ('<option value="' . $key . '" selected>' . $exp . '</option>' . "\n");
		else  print ('<option value="' . $key . '">' . $exp . '</option>' . "\n");
	}
	print ("</select>\n");
	print ("</td><td>\n");
