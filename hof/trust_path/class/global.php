...
function BattleLogDetail($log, $type = false)
{
	...
	// 勝利チームによって色を分けて表示
	if ($type == "RANK") print ("[ <a href=\"?rlog={$time}\">{$date}</a> ]&nbsp;\n");
	else
		if ($type == "UNION") print ("[ <a href=\"?ulog={$time}\">{$date}</a> ]&nbsp;\n");
		else  print ("[ <a href=\"?log={$time}\">{$date}</a> ]&nbsp;\n");
	print ("<span class=\"bold\">$act</span>turns&nbsp;\n"); //総ターン数
	if ($win === "0") print ("<span class=\"recover\">{$team[0]}</span>");
	else
		if ($win === "1") print ("<span class=\"dmg\">{$team[0]}</span>");
		else  print ("{$team[0]}");
	...
	if ($win === "0") print ("<span class=\"dmg\">{$team[1]}</span>");
	else
		if ($win === "1") print ("<span class=\"recover\">{$team[1]}</span>");
		else  print ("{$team[1]}");
...
function ShowSkillDetail($skill, $radio = false)
{
	...
	print ('<img src="' . HOF_Class_Icon::getIamgeUrl($skill["img"], IMG_ICON . 'skill/') . '" class="vcent">');
	print ("{$skill[name]}");
...
function ShowItemDetail($item, $amount = false, $text = false, $need = false)
{
	...
	$html = "<img src=\"" . HOF_Class_Icon::getIamgeUrl($item["img"], IMG_ICON . 'item/') . "\" class=\"vcent\">";
	// 精錬値
	...
	if ($need && $item["need"])
	{
		...
		foreach ($item["need"] as $M_itemNo => $M_amount)
		{
			...
			$html .= "<img src=\"" . HOF_Class_Icon::getIamgeUrl($M_item["img"], IMG_ICON) . "\" class=\"vcent\">";
