...
function LoadSkillTree($char)
{
	...
	*/
	...
	//////////////////////////////////// 剣技
	if ($char->job == 100 || $char->job == 101 || $char->job == 102 || $char->job == 103)
	{
		if ($lnd["1001"]) //Bash
 				$list[] = "1003"; //DowbleAttack
		if ($lnd["1001"]) $list[] = "1013"; //Stab
		if ($lnd["1001"]) $list[] = "3110"; //Reinforce
		if ($lnd["1001"]) $list[] = "3120"; //FirstAid
		if ($lnd["1003"]) //DowbleAttack
 				$list[] = "1017"; //RagingBlow
		if ($lnd["1003"]) $list[] = "1011"; //WeaponBreak
		if ($lnd["1013"]) //Stab
 				$list[] = "1014"; //FatalStab
		if ($lnd["1013"]) $list[] = "1016"; //ArmorPierce
	...
	if ($char->job == 102)
	{
		...
		if ($lnd["1114"] && $lnd["1117"] && $lnd["1102"]) $list[] = "1119"; // Possession
	}
	...
	//////////////////////////////////// 魔法系
	if ($char->job == 200 || $char->job == 201 || $char->job == 202 || $char->job == 203)
	{
		...
		if ($lnd["2021"]) //LightningBall
 				$list[] = "2022"; //Flash
		if ($lnd["2021"]) $list[] = "2023"; //Paralysis
	}
	...
	if ($char->job == 201)
	{
		...
		if ($lnd["2002"]) $list[] = "2003"; //Explosion
	...
	if ($char->job == 203)
	{
		...
		*/
		...
		if ($lnd["2463"]) $list[] = "2057"; // SelfMetamorphose
		if ($lnd["2462"]) // RaiseMummy
 				$list[] = "2464"; // GraveYard
		if ($lnd["2464"]) $list[] = "2056"; // ZombieRevival
		// ZombieControl + GraveYard
		if ($lnd["2463"] && $lnd["2464"]) $list[] = "2465"; // Biohazard
	}

	//////////////////////////////////// 支援系
	if ($char->job == 300 || $char->job == 301 || $char->job == 302)
	{
		if ($lnd["3000"])
		{ //Healing
			$list[] = "2100"; //Holy
	...
	if ($char->job == 301)
	{
		...
		if ($lnd["3102"] && $lnd["3220"] && $lnd["3230"]) $list[] = "3103"; //Sanctuary
	...
	if ($char->job == 302)
	{
		...
		if ($lnd["3250"] or $lnd["3255"]) $list[] = "3265"; //SpeedAsist
		$list[] = "3415"; //MagicCircle
	}
	//////////////////////////////////// 弓系
	if ($char->job == 400 || $char->job == 401 || $char->job == 402 || $char->job == 403)
	{
		$list[] = "2310"; //DoubleShot
		if (!$lnd["2300"]) $list[] = "2300"; //Shoot
	...
	if ($char->job == 401)
	{
		...
		if ($lnd["2302"] && $lnd["2305"] && $lnd["2306"]) $list[] = "2307"; //HurricaneShot
	}
	...
	if ($char->job == 403)
	{
		...
		if ($lnd["1208"]) $list[] = "1209"; //TransPoison
		$list[] = "1203"; //KnifeThrow
		if ($lnd["1203"]) $list[] = "1204"; //ScatterKnife
		$list[] = "1205"; //SulfaricAcid
		if ($lnd["1205"]) $list[] = "1206"; //AcidMist
	...
	//////////////////////////////////// 共通系
	if (19 < $char->level) $list[] = "4000"; //臨戦態勢
