	...
	*/
	...
	function Challenge($id)
	{
		...
		if (!$MyRank)
		{
			...
			if ($message == "Battle" && $result === true) $this->RankUp($MyID);
		...
		if ($MyRank)
		{
			...
			list($message, $result) = $this->RankBattle($MyID, $RivalID);
			if ($message != "Battle") return array($message, $result);
	...
	function RankUp($id)
	{
		...
		if ($place === 0 || $number < 2) return false;
	...
	function RankDown($id)
	{
		...
		$number = count($this->Ranking);
		if ($place === ($number - 1) || $number < 2) return false;
		...
		if (count($this->Ranking) < 1)
		{
			...
			if (is_numeric($from) && is_numeric($to))
			{
				for ($from; $from < $to; $from++)
				{
					...
					if ($this->Ranking["$from"]) echo ($from + 1) . "{$place} : " . $user->name . "<br />";
					//else break;
				}
				// 表示する数を指定されなかった時(全表示)
			}
