	...
	*/
	function __construct()
	{
		...
		while ($line = fgets($this->fp))
		{
			$line = trim($line);
			if ($line == "") continue;
			if (count($this->Ranking[$Place]) === $this->SamePlaceAmount($Place)) $Place++;
	...
	function ShowRanking($from = false, $to = false, $bold_id = false)
	{
		...
		{
			if (!$this->Ranking["$Place"]) break;
			print ("<tr><td class=\"td7\" valign=\"middle\" style=\"text-align:center\">\n");
			// 順位アイコン
			switch ($Place)
			{
				case 0:
					print ('<img src="' . HOF_Class_Icon::getIamgeUrl('crown01', IMG_ICON) . '" class="vcent" />');
					break;
				case 1:
					print ('<img src="' . HOF_Class_Icon::getIamgeUrl('crown02', IMG_ICON) . '" class="vcent" />');
					break;
				case 2:
					print ('<img src="' . HOF_Class_Icon::getIamgeUrl('crown03', IMG_ICON) . '" class="vcent" />');
					break;
				default:
	...
	function LoadUserName($id, $rank = false)
	{
		...
		if ($rank) return array($this->UserName["$id"], $this->UserRecord["$id"]);
		else  return $this->UserName["$id"];
	}
	//////////////////////////////////////////////////
	//
	function dump()
