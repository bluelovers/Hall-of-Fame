...
class Auction
{
	...
	function ItemCheckSuccess()
	{
		...
		foreach ($this->Article as $no => $Article)
		{
			...
			if ($Article["bidder"])
			{
				...
				// 結果をログに残せ
				$this->AddLog("No.{$Article[No]} <img src=\"" . HOF_Class_Icon::getIamge($item["img"], IMG_ICON) . "\"><span class=\"bold\">{$item[name]} x{$Article[amount]}</span>個 を " . $this->UserGetNameFromTemp($Article["bidder"]) . " が " . HOF_Helper_Global::MoneyFormat($Article["price"]) . " で<span class=\"recover\">落札しました。</span>");
			}
			else
			{
				// 入札が無かった場合、出品者に返却。
				...
				// 結果をログに残せ
				$this->AddLog("No.{$Article[No]} <img src=\"" . HOF_Class_Icon::getIamge($item["img"], IMG_ICON) . "\"><span class=\"bold\">{$item[name]} x{$Article[amount]}</span>個 は<span class=\"dmg\">入札者無しで流れました。</span>");
			}
			// 最後に消す
	...
	function ItemAddArticle($item, $amount, $id, $time, $StartPrice, $comment)
	{
		...
		$this->AddLog("No." . $this->ArticleNo . " に <img src=\"" . HOF_Class_Icon::getIamge($itemData["img"], IMG_ICON) . "\"><span class=\"bold\">{$itemData[name]} x{$amount}</span>個が<span class=\"charge\">出品されました。</span>");
		$this->DataChange = true;
	}
	//////////////////////////////////////////////
	//	オークションのデータを保存する
...
function BottomPrice($price)
{
	...
	if ($bottom < 101) return sprintf("%0.0f", $price + 100);
	else  return sprintf("%0.0f", $price + $bottom);
}
