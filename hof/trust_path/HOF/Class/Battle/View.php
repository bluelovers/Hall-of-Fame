		...
		<tr>
			<td class="teams">
				<div class="bold {$css_union[TEAM_1]}">
					{$this->battle->teams[TEAM_1]['team']->team_name}
				</div>
			...
			<td class="teams ttd1">
				<div class="bold {$css_union[TEAM_0]}">
					{$this->battle->teams[TEAM_0]['team']->team_name}
				</div>
...
EOM
	...
	function ShowResult($result)
	{
		...
		else
		{
			...
			$TeamName = $this->battle->teams[$result]['team']->team_name();
		...
		*/
		...
		//if ($this->battle->teams[TEAM_1]['exp'])
		if ($this->battle->teams[TEAM_1]['team']->data->reward['exp'])
		{
			//得た経験値
			echo ("TotalExp : " . $this->battle->teams[TEAM_1]['team']->data->reward['exp'] . "<br />\n");
		}
		if ($this->battle->teams[TEAM_1]['team']->data->reward['money'])
		{
			//得たお金
			echo ("Funds : " . HOF_Helper_Global::MoneyFormat($this->battle->teams[TEAM_1]['team']->data->reward['money']) . "<br />\n");
		}
		if ($this->battle->teams[TEAM_1]['team']->data->reward['item'])
		{
			//得たアイテム
			echo ("<div class=\"bold\">Items</div>\n");
			foreach ($this->battle->teams[TEAM_1]['team']->data->reward['item'] as $itemno => $amount)
			{
		...
		echo ("Alive : {$TotalAlive1}/" . count($this->battle->teams[TEAM_0]['team']) . "<br />\n");
		echo ("TotalDamage : ".$this->battle->teams[TEAM_0]['team']->data->log['dmg']."<br />\n");
		if ($this->battle->teams[TEAM_0]['team']->data->reward['exp'])
		{
			//得た経験値
			echo ("TotalExp : " . $this->battle->teams[TEAM_0]['team']->data->reward['exp'] . "<br />\n");
		}
		if ($this->battle->teams[TEAM_0]['team']->data->reward['money'])
		{
			//得たお金
			echo ("Funds : " . HOF_Helper_Global::MoneyFormat($this->battle->teams[TEAM_0]['team']->data->reward['money']) . "<br />\n");
		}
		if ($this->battle->teams[TEAM_0]['team']->data->reward['item'])
		{
			//得たアイテム
			echo ("<div class=\"bold\">Items</div>\n");
			foreach ($this->battle->teams[TEAM_0]['team']->data->reward['item'] as $itemno => $amount)
			{
				$item = HOF_Model_Data::getItemData($itemno);
				echo ("<img src=\"" . HOF_Class_Icon::getImageUrl($item["img"], HOF_Class_Icon::IMG_ITEM) . "\" class=\"vcent\">");
				echo ("{$item[name]} x {$amount}<br />\n");
