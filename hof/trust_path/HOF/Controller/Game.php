...
class HOF_Controller_Game extends HOF_Class_Controller
{
    ...
    public function _main_init()
    {
        $this->user = &HOF::user();
    }

    public function _main_input()
    {
        ...
        if ('logout' == $this->input->action || HOF::$input->request['logout'] || $this->input->logout) {
    ...
    public function _main_before()
    {
        parent::_main_before();

        if (!in_array($this->action, ['logout', 'check_login'])) {
            HOF::user()->CheckLogin();
        }

        if ('first_login' != $this->action && 'check_login' != $this->action) {
    ...
    public function _main_action_default($message = null)
    {
     ...
     * 初回ログイン用のフォーム.
     */
    public function _main_action_first_login()
    {
        if (true !== HOF::user()->CheckLogin()) {
    ...
    public function _main_action_delete_my_data()
    {
        if (true !== HOF::user()->CheckLogin()) {
            header('Location: ' . HOF::url());
            HOF::end();
        ...
        if (!$message = $this->DeleteMyData()) {
            //$this->_main_stop(true);
            $this->output->error['delete_my_data'][] = ['Error', 'margin15'];
        } else {
            $this->user->fpclose_all();
            $this->_main_exec('login', $message);
    ...
    public function _main_action_login($message = null)
    {
        if ('logout' != $this->input->action && true === HOF::user()->CheckLogin()) {
    ...
    public function _main_action_show_deleted_user()
    {
        $list = HOF_Model_Main::getNameDelList();

        $this->error = $this->output->error = [];
                    ...
                    if ($data = HOF_Class_Yaml::load($file)) {
                        ...
                        if (empty($this->error)) {
                            ...
                            try {
                                ...
                                unset($cache_user_del['user_del'][$id], $cache_user_del['name_del'][$id]);
                                ...
                                $this->output->msg_result[] = ["ID: {$id} , Recover ok now.\nYou can use this ID login."];
                            } catch (Exception $e) {
                                $this->_error('Error: ' . __LINE__ . '.');
                                break;
                            }
     ...
     * 新規ID作成用のフォーム.
     */
    public function _main_action_newgame()
    {
        if (true === HOF::user()->CheckLogin()) {
            $this->_main_exec('first_login');
    ...
    public function _error($s, $a = null)
    {
        $this->output->error[] = [$s, $a];
        $this->error[] = $s;
    }
    ...
    {
        if (empty($this->input->newid)) {
            return [false, 'Enter ID.'];
        }

        // 正規表現
        if (!preg_match('/^[0-9a-zA-Z]{4,16}$/', $this->input->newid) || preg_match('/[^0-9a-zA-Z]+/', $this->input->newid)) {
            return [false, 'Bad ID'];
        }
        ...
        // PASS
        if (empty($this->input->pass1) || empty($this->input->pass2)) {
            return [false, 'Enter both Password.'];
        }

        if (!preg_match('/^[0-9a-zA-Z]{4,16}$/', $this->input->pass1) || preg_match('/[^0-9a-zA-Z]+/', $this->input->pass1)) {
            return [false, 'Bad Password 1'];
        }
        if (strlen($this->input->pass1) < 4 || 16 < strlen($this->input->pass1)) { //文字制限
            return [false, 'Bad Password 1'];
        }
        if (!preg_match('/^[0-9a-zA-Z]{4,16}$/', $this->input->pass2) || preg_match('/[^0-9a-zA-Z]+/', $this->input->pass2)) {
            return [false, 'Bad Password 2'];
        }
        if (strlen($this->input->pass2) < 4 || 16 < strlen($this->input->pass2)) { //文字制限
            return [false, 'Bad Password 2'];
        }

        if ($this->input->pass1 !== $this->input->pass2) {
            return [false, 'Password dismatch.'];
        }

        $dir = HOF_Helper_Char::user_path($this->input->newid);

        if (is_dir($dir) || $msg = $this->is_registered($this->input->newid)) {
            return [false, is_string($msg) ? $msg : 'This ID has been already used.'];
        }
    ...
    public function FirstLogin()
    {
        ...
        foreach ($base_list as $i) {
            ...
            foreach (array_keys($jobdata['gender']) as $j) {
                $chars[$k] = HOF_Model_Char::newBaseChar($i, ['gender' => $j]);
        ...
        do {
            ...
            if ($this->input->recruit_no && $chars[$this->input->recruit_no] instanceof HOF_Class_Char_Abstract) {
                $job = $chars[$this->input->recruit_no]->job;

                $char = HOF_Model_Char::newBaseChar($job, ['name' => $this->input->char_name, 'gender' => $chars[$this->input->recruit_no]->gender]);
            } else {
                $this->_error('Select characters job.');
                break;
            }
    ...
    public function _main_action_logout($message = null)
    {
        if ('login' == $this->input->action) {
            return;
        }
        ...
        $this->_main_exec('login', 'logout' == $this->input->action ? 'User Logout!!' : $message);
    }

    public function _main_action_check_login()
    {
    ...
    public function _main_action_setting()
    {
    ...
    public function SettingProcess()
    {
        ...
        if ($this->input->color) {
            if (6 != strlen($this->input->color) && !preg_match('^[0369cf]{6}', $this->input->color)) {
    ...
    public function _main_action_setting_rename()
    {
        if ($this->user_rename()) {
            $this->user->SaveData();
        }
    ...
    protected function user_rename()
    {
        ...
        if ($this->input->NewName || $this->input->setting_rename) {
            if (!$this->user->TakeMoney(NEW_NAME_COST)) {
                $this->output->error['setting_rename'][] = ['money not enough'];
            ...
            if (!$NewName || !$this->input->NewName || $NewName != $this->input->NewName) {
                $this->output->error['setting_rename'][] = ['Bad Name.'];
            ...
            if (in_array($NewName, $userName)) {
                $this->output->error['setting_rename'][] = ["{$NewName} は、名前は既に使用されている。", 'margin15'];
            ...
            if ($this->user->ChangeName($NewName, true)) {
                $this->output->msg_result['setting_rename'][] = ["Name Changed ({$OldName} -> {$NewName})", 'margin15'];

                return true;
            }

            $this->output->error['setting_rename'][] = ['?']; //名前が同じ？
            return false;
        }
    }
}
